...
def run_assistant(
    ...
    while True:
        ...
        cost_stats = estimate_cost(user_msg, full_reply, model=OPENAI_MODEL)
...
def format_comparison_table(results: list[dict]) -> str:
    ...
    for r in results:
        ...
        g_res = (r.get("gpt4o_response", "")[:37] + "...") if len(r.get("gpt4o_response", "")) > 40 else r.get("gpt4o_response", "")
        m_res = (r.get("mini_response", "")[:37] + "...") if len(r.get("mini_response", "")) > 40 else r.get("mini_response", "")
        g_lat = f"{r.get('gpt4o_latency', 0.0):.2f}s"
        m_lat = f"{r.get('mini_latency', 0.0):.2f}s"
        lines.append(f"{p:<40} | {g_res:<40} | {m_res:<40} | {g_lat:<14} | {m_lat:<12}")
    return "\n".join(lines)
